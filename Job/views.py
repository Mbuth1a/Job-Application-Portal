...
def user_home(request):
    ...
    user = request.user
    student = StudentUser.objects.filter(user_id=user.id)    
    error = ""
    if request.method =='POST':
        f = request.POST['fname'] 
        l = request.POST['lname'] 
...
def user_signup(request):
    ...
    if request.method =='POST':
        ...
        try:
          user=User.objects.create_user(first_name=f,last_name=l, username=e, password=p)
          StudentUser.objects.create(user=user, mobile=con, resume=r,gender=gen , type="student")
          error="no"
        except:
            error="yes"
...
def delete_recruiter(request,pid):
    ...
    recruiter.delete()
    return render(request,'recruiter_all.html')
...
def edit_jobdetail(request,pid):
    ...
    d={'error':error, 'job':job}
    return render(request,'edit_jobdetail',d)   
...
def change_companylogo(request,pid):
    ...
    return render(request,'change_companylogo',d)    
...
def job_list(request):
    ...
    user = request.user
    recruiter = Recruiter.objects.filter(user=user).first()
    job = Jobs.objects.filter(recruiter=recruiter).first()
    d = {'job':job}
